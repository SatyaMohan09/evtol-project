...
import birdData from "../data/dynamic_obstacle 2.json";
...
const UnifiedVisualizationScene = forwardRef(
  ({ cameraMode = "OVERVIEW", missionMode = true, onPositionUpdate }, ref) => {
    ...
    const evtolRef = useRef();
    const fpvRef = useRef();
    const bottomCamPivotRef = useRef();
    const bottomCamRef = useRef();

    const bottomCamRotation = useRef({
      yaw: 0,
      pitch: 0,
    });
      ...
      async function loadTrajectory() {
        try {
          const res = await fetch(
            "http://localhost:8080/api/dynamic-test",
          );
    ...
    useEffect(() => {
      ...
      // ===== BIRD PATH SETUP =====
      birdPathRef.current = birdData.map(
        (p) => new THREE.Vector3(p.x, p.y, p.z),
      );
...
// ===== BOTTOM CAMERA MOUSE CONTROL =====
let isDragging = false;

const onMouseDown = () => {
  if (cameraMode === "BOTTOM_CAM") {
    isDragging = true;
  }
};
...
const onMouseMove = (e) => {
  if (!isDragging || cameraMode !== "BOTTOM_CAM") return;

  bottomCamRotation.current.yaw -= e.movementX * 0.003;

  bottomCamRotation.current.pitch -= e.movementY * 0.003;

  // limit vertical rotation
  bottomCamRotation.current.pitch = THREE.MathUtils.clamp(
    bottomCamRotation.current.pitch,
    -Math.PI / 2,
    Math.PI / 2,
  );
};
...
window.addEventListener("mousemove", onMouseMove);
      ...
      //BIRD Model loading and orientation
      ...
      // Load textures (grass + displacement)
      const textureLoader = new THREE.TextureLoader();
      ...
      buildingLoader.load("/models/Skyscraper.glb", (gltf) => {
        ...
        buildingList.forEach((b) => {
          ...
          // ===== PLACE USING CENTER COORDINATES =====
          building.position.set(b.x, b.y, b.z);
      ...
      evtolGroup.add(fpvOffset);
      // ===== BOTTOM CAMERA RIG =====
const bottomCamPivot = new THREE.Object3D();

// under the drone
bottomCamPivot.position.set(0, -15, 0);

evtolGroup.add(bottomCamPivot);

bottomCamPivotRef.current = bottomCamPivot;

// actual camera attachment point
const bottomCam = new THREE.Object3D();

// slightly below body
bottomCam.position.set(0, -5, 0);

bottomCamPivot.add(bottomCam);

bottomCamRef.current = bottomCam;
      ...
      function addCampFence(baseModel) {
        ...
        //  RECTANGLE DIMENSIONS
        const halfWidth = 1580; // X (left-right)
        const frontDepth = 4800; // Z+ (front side)
        const backDepth = 1500; // Z- (behind tent)
      ...
      function addRockPath(baseModel) {
        ...
        const pathWidth = 120; // slight width
      ...
      hospitalArea.scale.set(1.3, 1.3, 1.3);
      scene.add(hospitalArea);
      ...
      function animate(time) {
        ...
        if (sim.current.isRunning) {
          ...
          if (distToPad > 200) {
            sim.current.waitTimer = 0;
            sim.current.index += sim.current.playbackSpeed * 60 * delta;
          } else if (distToPad > 20) {
            sim.current.index += sim.current.playbackSpeed * 20 * delta;
        ...
        if (camera && controls && evtol) {
          ...
          if (cameraMode === "FPV" && fpvRef.current) {
            ...
            controls.enableZoom = false;
            controls.enablePan = false;
          }if (cameraMode === "BOTTOM_CAM" && bottomCamRef.current) {
  const worldPos = new THREE.Vector3();
  const droneQuat = new THREE.Quaternion();

  bottomCamRef.current.getWorldPosition(worldPos);
  evtol.getWorldQuaternion(droneQuat);

  // apply drone rotation
  const yawQuat = new THREE.Quaternion().setFromAxisAngle(
    new THREE.Vector3(0, 1, 0),
    bottomCamRotation.current.yaw,
  );
  ...
  controls.enablePan = false;
        ...
        const path = birdPathRef.current;

        if (bird && path.length > 1) {
          const speed = 10.0;

          // movement
          birdIndexRef.current += birdDirectionRef.current * speed * delta * 60;

          // reverse at ends
          if (birdIndexRef.current >= path.length - 1) {
            birdIndexRef.current = path.length - 1;
          ...
          const idx = Math.floor(birdIndexRef.current);
          const t = birdIndexRef.current - idx;
          ...
          if (p1 && p2) {
            ...
            // FORWARD
            if (birdDirectionRef.current === 1) {
              direction = p2.clone().sub(p1).normalize();
            }

            // REVERSE
            else {
              direction = p1.clone().sub(p2).normalize();
            }
